...
export class EmployeeService {
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    const copy: IEmployee = Object.assign({}, employee, {
      employeeCinDeliveredDate:
        employee.employeeCinDeliveredDate != null && employee.employeeCinDeliveredDate.isValid()
          ? employee.employeeCinDeliveredDate.format(DATE_FORMAT)
          : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.employeeCinDeliveredDate = res.body.employeeCinDeliveredDate != null ? moment(res.body.employeeCinDeliveredDate) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((employee: IEmployee) => {
        employee.employeeCinDeliveredDate = employee.employeeCinDeliveredDate != null ? moment(employee.employeeCinDeliveredDate) : null;
      });
    }
    return res;
  }
