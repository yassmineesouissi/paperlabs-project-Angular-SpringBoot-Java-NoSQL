...
public class UserResource {
    ...
    /**
     * {@code SEARCH  /_search/users-fields?query=:query} : search for the user corresponding
     * to the query.
     *
     ...
     * @return the result of the search.
     */
    @GetMapping("/_search/users-fields")
    public ResponseEntity<List<UserDTO>> searchLegalDocuments(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of User for query {}", query);
        Page<UserDTO> page = userService.searchUsersWithFields(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
